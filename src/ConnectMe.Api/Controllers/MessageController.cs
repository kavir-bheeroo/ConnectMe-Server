...
namespace ConnectMe.Api.Controllers
{
    ...
    public class MessageController : Controller
    {
        ...
        public async Task<IActionResult> SendNotificationMessageToCloud([FromBody]SendMessageRequest request)
        {
            ...
            var resultCode = await _messagingService.SendNotificationMessage(currentUser.Id, request.ToUserId, request.ReceiverToken, request.Title, request.Body);
        ...
        {
            var currentUser = await _userManager.GetUserAsync(HttpContext.User);
            var resultCode = await _messagingService.SendDataMessage(currentUser.Id, request.ToUserId, request.ReceiverToken, request.Title, request.Body);
